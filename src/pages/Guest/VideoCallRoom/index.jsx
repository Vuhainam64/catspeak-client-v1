...
import { useGetVideoSessionByIdQuery, useLeaveVideoSessionMutation } from '@/store/api/videoSessionsApi'
...
const VideoCallRoom = () => {
  ...
  const { data: session } = useGetVideoSessionByIdQuery(id, {
    skip: !id,
  })
  
  const [leaveSession, { isLoading: isLeaving }] = useLeaveVideoSessionMutation()
  ...
  return (
    <div className="flex h-full w-full flex-col bg-[#1f1f1f] text-gray-100">
      ...
      <div className="flex items-center justify-between border-b border-[#303134] bg-[#202124] px-6 py-4">
        ...
        <div className="flex items-center gap-4">
          {/* Meeting ID */}
          <button className="flex items-center gap-2 rounded-lg bg-blue-500 px-3 py-1.5 text-sm font-semibold text-white">
      ...
      <div className="flex items-center justify-between border-t border-[#303134] bg-[#202124] px-6 py-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {cameraOn ? <FiVideo className="h-5 w-5" /> : <FiVideoOff className="h-5 w-5" />}
          </button>
          ...
          {/* Participants Toggle */}
           <button 
                ...
                className={`flex h-12 w-12 items-center justify-center rounded-full transition ${
                    showParticipants ? 'bg-[#8ab4f8] text-[#202124]' : 'bg-[#3c4043] text-white hover:bg-[#5f6368]'
                }`}
            >
          ...
          <button
            ...
            className={`flex h-12 w-12 items-center justify-center rounded-full transition ${
              showChat ? 'bg-[#8ab4f8] text-[#202124]' : 'bg-[#3c4043] text-white'
            }`}
          >
